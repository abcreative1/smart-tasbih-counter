...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const loadedTasbihs = loadTasbihs();
    setTasbihs(loadedTasbihs);
    
    // Load navigation state (resume where left off)
    const appState = loadAppState();
    if (appState.activeTasbihId && loadedTasbihs.find(t => t.id === appState.activeTasbihId)) {
        setActiveTasbihId(appState.activeTasbihId);
